...
let S={profile:{first:"",last:"",age:"",net:0,daily:8,weekly:40,leave:21,otWeek:175,otWeekend:200,weekendPremium:0,saturdayIncluded:0,currency:"lei"},days:{},months:{}},D=new Date(),selected="";
D.setDate(1);
...
function stats(y,m){
  ...
  const s={days:0,hours:0,ot:0,co:0,cm:0,abs:0,bonus:0,otPay:0,cmPay:0,satIncluded:0,satExtra:0,satExtraHours:0};
  Object.entries(S.days).forEach(([k,v])=>{
    ...
    if(v.type==="work"){
      ...
      const h=hours(v);
      s.hours+=h;
      s.bonus+=Number(v.dayBonus)||0;
      if(d.getDay()===6){
        const mode=v.saturdayMode||"included";
        if(mode==="extra"){
          s.satExtra++;
          s.satExtraHours+=h;
          s.ot+=h;
          ...
          s.satIncluded++;
          s.ot+=Math.max(0,h-(Number(S.profile.daily)||8));
        }
      }else{
        s.ot+=Number(v.ot)||0;
      }
  ...
  Object.entries(S.days).forEach(([k,v])=>{
    ...
    const d=date(k),h=hours(v),daily=Number(S.profile.daily)||8;
    let otHours=0,ratePct=Number(S.profile.otWeek)||175;
    if(d.getDay()===6){
      if((v.saturdayMode||"included")==="extra"){
        // Extra Saturday: every worked hour is paid at 200%.
        otHours=h;
      }else{
        // Included Saturday: only hours beyond the daily schedule are OT.
        otHours=Math.max(0,h-daily);
      }
      ratePct=200;
    }else{
      otHours=Number(v.ot)||0;
    }
    s.otPay+=otHours*s.rate*ratePct/100;
  });
...
function openDay(k){selected=k;let v=S.days[k]||{type:"work",start:"09:00",end:"17:00",break:0,ot:0,cmPct:75,dayBonus:0,saturdayMode:(date(k).getDay()===6&&(Number(S.profile.saturdayIncluded)||0)<=0?"extra":"included"),note:""};$("modalTitle").textContent=date(k).toLocaleDateString("ro-RO",{weekday:"long",day:"numeric",month:"long"});["type","start","end","break","ot","cmPct","dayBonus","saturdayMode","note"].forEach(id=>$(id).value=v[id]??"");$("saturdayModeWrap").style.display=date(k).getDay()===6?"":"none";$("rateHint").textContent=date(k).getDay()===6
  ?"Sâmbătă inclusă: doar orele peste norma zilnică sunt OT la 200%. Sâmbătă extra: toate orele zilei sunt OT la 200%."
  :`OT pentru această zi: ${S.profile.otWeek}%`;show("modal")}
function show(id){$(id).classList.remove("hidden")}function hide(id){$(id).classList.add("hidden")}
$("save").onclick=()=>{
  const current={type:$("type").value,start:$("start").value,end:$("end").value,break:+$("break").value||0,ot:+$("ot").value||0,cmPct:+$("cmPct").value||0,dayBonus:+$("dayBonus").value||0,saturdayMode:$("saturdayMode").value||"included",note:$("note").value};
  const d=date(selected);
  if(d.getDay()===6&&current.type==="work"&&current.saturdayMode==="included"){
    const maxIncluded=Math.max(0,Math.min(5,Number(S.profile.saturdayIncluded)||0));
    const monthPrefix=selected.slice(0,7);
    const alreadyIncluded=Object.entries(S.days).filter(([k,v])=>k.startsWith(monthPrefix)&&k!==selected&&v.type==="work"&&date(k).getDay()===6&&(v.saturdayMode||"included")==="included").length;
    if(alreadyIncluded>=maxIncluded){
      alert(`Ai setat ${maxIncluded} sâmbete incluse în salariul de bază pentru această lună. Pentru o sâmbătă suplimentară selectează „Sâmbătă extra”.`);
      return;
    }
  }
  S.days[selected]=current;save();hide("modal");render()
};
...
function openSettings(){let p=S.profile;["first","last","age","net","daily","weekly","leave","otWeek","otWeekend","weekendPremium","saturdayIncluded","currency"].forEach(id=>$(id).value=p[id]??"");show("settingsModal")}
$("saveSettings").onclick=()=>{["first","last","currency"].forEach(id=>S.profile[id]=$(id).value);["age","net","daily","weekly","leave","otWeek","otWeekend","weekendPremium","saturdayIncluded"].forEach(id=>S.profile[id]=Number($(id).value)||0);save();hide("settingsModal");render()};
...
function loadMonthInputs(){let x=S.months[monthKey()]||{};$("bonus").value=x.bonus||0;$("deductions").value=x.deductions||0;$("dailyAllowance").value=x.dailyAllowance||0;$("nightHours").value=x.nightHours||0;$("nightRate").value=x.nightRate||25;let s=stats(D.getFullYear(),D.getMonth());$("salaryBreakdown").innerHTML=[["Salariu bază estimat",fmt(s.normal*s.rate)],["OT",fmt(s.otPay)],["Sâmbete incluse",s.satIncluded+" zile"],["Sâmbete extra",s.satExtra+" zile • "+fmt(s.satExtraHours)+" h"],["CO",fmt(s.co*s.rate*S.profile.daily)],["CM",fmt(s.cmPay*s.rate*S.profile.daily)],["Bonusuri",fmt(s.bonus)],["Rețineri",fmt(-(x.deductions||0))],["TOTAL NET ESTIMAT",fmt(s.net)]].map(x=>`<div><span>${x[0]}</span><strong>${x[1]} ${S.profile.currency}</strong></div>`).join("")}
